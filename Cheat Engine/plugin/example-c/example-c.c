...
DWORD __stdcall suspendloadpeThread(LPVOID lp) {
	...
	//在OEP下断 
	HMODULE firstModule = ProCntrl::GetModule(proinfo.hProcess, "");//getBaseName(filePath)
	std::string realPePath = ProCntrl::GetModulePath(proinfo.hProcess, firstModule);
	free(peDos);
	peDos = ProCntrl::readPEFile(realPePath.c_str());
	__int64 baseAddress = (__int64)firstModule;
	if (baseAddress) {
...
void __stdcall mainmenuplugin(void) {
	...
	//int r = MessageBoxA(NULL, "[Yes] to load CE driver or [No] to load console [Cancel] to load my VT", "WARNING", MB_YESNOCANCEL);
	int r = atoi(luaLoadScriptWithReturn(messageBoxScript))+1;
	if (r == IDOK) {
		if (luaToolbox_record) {
			int rType = Exported.memrec_getType(luaToolbox_record);//release和debug模式下编译的CE在此处的返回值rType是不同的
			Exported.memrec_delete(luaToolbox_record);
			luaToolbox_record = NULL;
			if(rType >0) return;

		}

		luaToolbox_record = luaCreateMemoryRecord();
		...
		for (int i = 0; i < luaScriptList.size(); i++)
		{
			...
			addScriptToTable(getDynamicChars(it.first), getDynamicChars(it.second), luaToolbox_record);
	...
	else if(r== IDABORT && launcher == NULL && fileExist(get_cwd_file(nowPath, "\\DBKKernel\\shv.sys")))	{

		if (luaGetCurrentDebuggerInterface() != Windows) return luaPrintf("[WARNNING] we find ce not active windows debbuger in register so shv driver will be disable\n");
		launcher = new SysLauncher(nowPath, "shv");
		if (launcher->installDvr())luaPrintf("install VT driver success.\n");
		if (launcher->startDvr() && launcher->connectToIoDevice())luaPrintf("start VT success.\n");
		PointersReassigned(0);

	}else if(r== IDRETRY && launcher == NULL && fileExist(get_cwd_file(nowPath, "\\DBKKernel\\MyDriver.sys"))) {
		launcher = new SysLauncher(nowPath, "MyDriver");
		if (launcher->installDvr())luaPrintf("install MyDriver success.\n");
		if (launcher->startDvr()) luaPrintf("start MyDriver success.\n");
	}
	else if (r== IDIGNORE) {
		...
		InitMyCeMiniUi("Show Mini UI", false);
	}
}
...
int lua_plugin_utf8_to_gbk(lua_State* L) //make sure this is cdecl
{
	int type = lua_type(L, -1);
	const char* nowPath;
	if (type == LUA_TSTRING) {//nil
		nowPath = lua_tostring(L, -1);

	}
	else {
		nowPath = "";
	}
	std::string filepath = UTF8ToGbk(nowPath);
...
BOOL __stdcall CEPlugin_InitializePlugin(PExportedFunctions ef, int pluginid)
{
	...
	lua_register(lua_state, "lua_plugin_utf8_to_gbk", lua_plugin_utf8_to_gbk);
	luaRegisterLuaFunctionHighlight("lua_plugin_utf8_to_gbk");
	...
	{	
		...
		if (launcher->stopDvr())
		{
			...
			if (launcher->unloadDvr())
			{
				...
				MessageBoxA(0, "unload fail", "unload fail", MB_OK | MB_ICONEXCLAMATION);//警告
			}
		}
		else
		{
			MessageBoxA(0, "stop fail", "unload fail", MB_OK | MB_ICONHAND);//红叉
		}
		
	}
	if(Exported.DBKResumeProcess(GetCurrentProcessId())==TRUE) { //如果加载了DBK驱动，则退出时需要调用
		// 相对路径
		system("start kernelmoduleunloader-x86_64.exe");
	}
