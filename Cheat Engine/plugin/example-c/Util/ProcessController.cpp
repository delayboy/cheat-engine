	...
	if (runover)
	{
		for (i = 0; i <= (cbNeeded / sizeof(HMODULE)); i++)
		{
			...
			if (GetModuleFileNameExA(pHandle, hMods[i], szModName, sizeof(szModName) / sizeof(TCHAR)))
			{
				...
				//you will need to change this to the name of the exe of the foreign process
				printf("module:%s\n", wstrModName.c_str());
				if(basename.empty()) return hMods[i];
				if (wstrModName.find(basename) != std::string::npos)
				{
...
std::string ProCntrl::GetModulePath(HANDLE hProcess,HMODULE hModule)
{
	char path[MAX_PATH];
	DWORD result = GetModuleFileNameExA(hProcess, hModule, path, MAX_PATH);

	if (result == 0) {
		DWORD error = GetLastError();
		std::cout << "GetModuleFileNameEx failed with error: " << error << std::endl;
		return "";
	}
	return path;
}
