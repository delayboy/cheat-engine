...
const char* messageBoxScript = "local string_list = createStringList();\n\
string_list.add('[dkjson.lua] My Lua Script Box');\n\
string_list.add('[example-c.dll] Console Switch');\n\
string_list.add('[shv.sys] VT Debugger');\n\
string_list.add('[MyDriver.sys] ObCallback Privilege Escalation');\n\
string_list.add('[myVT64.sys] Cheatengine DBVM');\n\
string_list.add('[MyCeMiniUi] call mini script');\n\
local select_index,struct_name=showSelectionList('Reinforcement Options','Advanced Debugger Add-ons',string_list,false);\n\
return string.format('%d', select_index); \n";
...
TTreeNode* luaTreeViewSelected(TCustomTreeView* treeView)
{
	return 	(TTreeNode*)luaGetTableSubMember(treeView, "TCustomTreeView", 1, "TTreeNode", "Selected");
}
...
int luaGetFormCount()
{
	const char* argTypes[1] = {"int" };
	return (int)luaFuncHandler("getFormCount", argTypes, 0, 1);
}
TForm* luaGetForm(int index)
{
	const char* argTypes[2] = { "int","TForm"};
	return (TForm*)luaFuncHandler("getForm", argTypes, 1, 1,index);
}
char* luaGetFormClassName(TForm* form)
{
	return 	(char*)luaGetTableSubMember(form, "TForm", 1, "string", "ClassName");
}
int luaGetFormComponentCount(TForm* form)
{
	return 	(int)luaGetTableSubMember(form, "TForm", 1, "int", "ComponentCount");
}
TComponent* luaGetFormComponent(TForm* form,int index)
{
	__setConstChar("int", "TComponent");
	TComponent* comp = (TComponent*)luaRunTableSubFunc(form, "TForm", "getComponent", argTypesTemp, 1, 1, index);
	return comp;
}
char* luaGetComponentName(TComponent* comp)
{
	return 	(char*)luaGetTableSubMember(comp, "TComponent", 1, "string", "Name");
}
TComponent* luaGetTraceFile()
{
	TComponent* comp = NULL;
	int form_count = luaGetFormCount();
	for (int i = 0; i < form_count; i++) {
		TForm* frm = luaGetForm(i);
		if (strcmpi(luaGetFormClassName(frm), "TfrmTracer") == 0) {
			int comp_count = luaGetFormComponentCount(frm);
			for (int j = 0; j < comp_count; j++) {
				comp = luaGetFormComponent(frm, j);
				if (strcmpi(luaGetComponentName(comp), "lvTracer") == 0) {
					return comp;
				}
			}
		}
...
PVOID addScriptToTable(char* title, char* script, PVOID parent_record)
{
	...
	if (parent_record) {
		...
		Exported.memrec_setDescription(parent_record, "My Lua Script Toolbox");
