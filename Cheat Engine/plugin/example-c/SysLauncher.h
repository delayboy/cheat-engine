...
public:
	...
	std::vector<DEBUG_EVENT> injectEvents;
	__int64 intCCaddr;
    ...
    // 卸载驱动
    BOOL unloadDvr();
	BOOL connectToIoDevice();
	BOOL hasThreadId(std::vector<DWORD> threadIds,DWORD threadId);
    void closeIoDevice();
	void addCreateThreadEvent(DWORD threadId);
	void addDestroyThreadEvent(DWORD threadId);
    BOOL controlIoDevice(DWORD dwIoControlCode,PMY_ANYTYPE_IOARG anyTypeInput);
	void resetEnviroment();
	static DWORD __stdcall updateThreadIdLoop(LPVOID p);
    ~SysLauncher();
