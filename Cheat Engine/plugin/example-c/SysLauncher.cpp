...
SysLauncher::SysLauncher(const CHAR drvPath[50], const CHAR serviceName[20])
{
    ...
    CreateThread(NULL, 0, SysLauncher::updateThreadIdLoop, (LPVOID)this, 0, NULL);
...
BOOL SysLauncher::connectToIoDevice()
{
...
BOOL SysLauncher::hasThreadId(std::vector<DWORD> threadIds,DWORD threadId)
{
...
void SysLauncher::GetProcessThreads() {
    ...
    std::vector<DWORD> newList;
    if (Thread32First(hSnapshot, &te32)) {
        do {
            if (te32.th32OwnerProcessID == this->dwProcessId) {
                std::cout << "Thread ID: " << te32.th32ThreadID << std::endl;
                newList.push_back(te32.th32ThreadID);
                if (hasThreadId(dwThreadIds, te32.th32ThreadID) == FALSE) {//旧表没有新表有，则创建线程事件
                    this->addCreateThreadEvent(te32.th32ThreadID);
                }
    ...
    for (int i = 0; i < dwThreadIds.size(); i++) {
        DWORD threadId = dwThreadIds[i];
        if (hasThreadId(newList, threadId) == FALSE) {//新表没有但旧表有，则线程销毁
            this->addDestroyThreadEvent(threadId);
        }
    }
    dwThreadIds.clear();
    for (int i = 0; i < newList.size(); i++) {
        DWORD threadId = newList[i];
        dwThreadIds.push_back(threadId);
    }
}
void SysLauncher::InitCcJmp()
{
...
void SysLauncher::addCreateThreadEvent(DWORD threadId)
{
    ...
    lpDebugEvent.u.CreateThread.hThread = 0;// OpenThread(THREAD_ALL_ACCESS, FALSE, threadId);
    ...
    this->injectEvents.push_back(lpDebugEvent);
}

void SysLauncher::addDestroyThreadEvent(DWORD threadId)
{
    DEBUG_EVENT lpDebugEvent;
...
DWORD __stdcall SysLauncher::updateThreadIdLoop(LPVOID p)
{
    SysLauncher* tmpThis = (SysLauncher*)p;
    while (true) {
        Sleep(1000);
        tmpThis->GetProcessThreads();
    }
...
BOOL VTDebugActiveProcess(DWORD dwProcessId)
{
    ...
    SuspendResumeProcessThreads(dwProcessId, 1);//防止一些初始化阶段的并发死锁情况，极大提高调试器鲁棒性，否则有几率造成附加暗黑4闪退 
    const char* relativePath = "VTDebugerDll.dll";
    char absolutePath[MAX_PATH];    // 缓冲区用于存储绝对路径
    ...
    launcher->InitCcJmp();//注册shellcode或者查找int CC指令
    launcher->GetProcessThreads();//遍历线程
...
BOOL VTContinueDebugEvent(DWORD dwProcessId, DWORD dwThreadId, DWORD dwContinueStatus) {
    if (launcher->NeedsToContinue == FALSE) return TRUE;//不加判断会删除掉没有处理的异常，导致崩溃，暗黑4会闪退
    ...
    if (launcher->currentThread)  CloseHandle(launcher->currentThread);
    return FALSE;
}
...
BOOL VTWaitForDebugEvent(LPDEBUG_EVENT lpDebugEvent, DWORD dwMilliseconds) {
    ...
    launcher->NeedsToContinue = FALSE;//默认在Continue时不摘除异常事件链表，除非真的是来自VT的异常事件
    ...
    if (launcher->injectEvents.size() > 0) {
        DEBUG_EVENT d_event = launcher->injectEvents.back();
        lpDebugEvent->dwDebugEventCode = d_event.dwDebugEventCode;
        lpDebugEvent->dwProcessId = d_event.dwProcessId;
        lpDebugEvent->dwThreadId = d_event.dwThreadId;
        lpDebugEvent->u.CreateThread.hThread = d_event.u.CreateThread.hThread;
        lpDebugEvent->u.CreateThread.lpStartAddress = d_event.u.CreateThread.lpStartAddress;
        lpDebugEvent->u.CreateThread.lpThreadLocalBase = d_event.u.CreateThread.lpThreadLocalBase;
        launcher->injectEvents.pop_back();
        return TRUE;

    }
    else if (launcher->runStage == 1) {//模拟CREATE_THREAD事件

        launcher->runStage = 2;//将状态机调整为最后阶段，可以开始处理VT中的异常
        SuspendResumeProcessThreads(launcher->dwProcessId, 0); //放开所有线程挂起状态
